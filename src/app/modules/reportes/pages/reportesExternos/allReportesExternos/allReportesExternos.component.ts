...
//   ******   COMPONENTES   ******  //
import { SaveReporteExternoComponent } from '../saveReporteExterno/saveReporteExterno.component';
import { DeleteReporteExternoComponent } from '../deleteReporteExterno/deleteReporteExterno.component';
import { ModalReporteExternoComponent } from '../modalReporteExterno/modalReporteExterno.component';

import { UsuariosReportesExternosComponent } from '../usuariosReportesExternos/usuariosReportesExternos.component';
...
@Component({
  selector: 'app-allReportesExternos',
  templateUrl: './allReportesExternos.component.html',
  standalone: false,
})
export class AllReportesExternosComponent implements OnInit, OnDestroy{
...
public selectedRow: ReporteExternoModel | null = null;
  ...
  constructor(
    ...
    private _reporteExterno: ReporteExternoService,
  ){
    ...
    //   ******   LISTADO DE DATOS   ******  //
    async allReportesExternos() {
      try {
          this._loadingService.setLoading(true); 
          let res: any = await firstValueFrom(this._reporteExterno.allReportesExternos());
    ...
    addReporteExterno() {
      if (!this._seguridadService.isexpired()) {    
          const modalRef = this.modal.open(SaveReporteExternoComponent, { centered: true, size: "xs", backdrop: "static", keyboard: false,});
          ...
          modalRef.componentInstance.registrosE.pipe(takeUntil(this.unsubscribe$)).subscribe({
              next:  (response: any) => {  this.reporteExternoModel = response; },
    ...
    initializeGrid(): void {
      this.columnDefs = [
...

        {
          ...
          cellRenderer: ButtonAccionReporteExterno,
          pinned: 'right',
          minWidth: 110,
          maxWidth: 110,
          suppressMenu: true,
...
usuariosReportesExternos(){
  if (!this.selectedRow) return;
  
  if (!this._seguridadService.isexpired()) {    
    const modalRef = this.modal.open(UsuariosReportesExternosComponent, { 
      centered: true, 
      size: "xl", 
      backdrop: "static", 
      keyboard: true
  ...
  modalRef.componentInstance.tablaNombre = 'reporteExterno';
...
@Component({
  ...
  template: `
      <app-action-buttons 
        [accesoModel]="AllReportesExternosComponent.accesoModel"
        [buttonView] = true
        [buttonEdit] = true
        [buttonClone] = true
        [buttonDelete] = true
        (view)="viewReporteExterno()"
        (edit)="editReporteExterno()"
        (clone)="clonReporteExterno()"
        (delete)="deleteReporteExterno()">
      </app-action-buttons>
  `,
})

export class ButtonAccionReporteExterno implements OnDestroy{
  ...
  constructor(
      ...
      public AllReportesExternosComponent: AllReportesExternosComponent
  ){}
  ...
  clonReporteExterno() {
      const modalRef = this.modalService.open(SaveReporteExternoComponent, { centered: true, size: 'xs',  backdrop: 'static',  keyboard: false });
      modalRef.componentInstance.registro_selected = this.params.data;
      modalRef.componentInstance.accion = 'clon';

      modalRef.componentInstance.registrosE.pipe(takeUntil(this.unsubscribe$)).subscribe({
        next: (response: any) => {  this.AllReportesExternosComponent.reporteExternoModel = response;  },
        error: (error: any) => { console.error(error.message); }
      });
  }
  ...
  editReporteExterno() {
      ...
      modalRef.componentInstance.registrosE.pipe(takeUntil(this.unsubscribe$)).subscribe({
          next: (response: any) => {
              let INDEX = this.AllReportesExternosComponent.reporteExternoModel.findIndex(registro => registro.id == response.id);
              let rowNode = this.AllReportesExternosComponent.gridApi.getRowNode(INDEX.toString());
              rowNode?.setData(this.AllReportesExternosComponent.reporteExternoModel[INDEX] = response);
          },
          error: (error: any) => {
  ...
  viewReporteExterno() {
      // const modalRef = this.modalService.open(SaveReporteExternoComponent, { centered: true, size: 'xs',  backdrop: 'static',  keyboard: true   });
      // modalRef.componentInstance.registro_selected = this.params.data;
      // modalRef.componentInstance.accion = 'view';

    const modalRef = this.modalService.open(ModalReporteExternoComponent, {
      centered: true,
      size: 'xxl',
      backdrop: 'static',
      keyboard: true,
  ...
  deleteReporteExterno() {
      const modalRef = this.modalService.open(DeleteReporteExternoComponent, { centered: true, size: 'md',  backdrop: 'static',  keyboard: true });      
      modalRef.componentInstance.registro_selected = this.params.data;
      modalRef.componentInstance.registrosE.pipe(takeUntil(this.unsubscribe$)).subscribe({
          next: (response: any) => {
            const selectedData = this.AllReportesExternosComponent.gridApi.getSelectedRows();
            const resdel = this.AllReportesExternosComponent.gridApi.applyTransaction({ remove: selectedData })!;
          },
          error: (error: any) => {
            console.error(error.message);
          }
