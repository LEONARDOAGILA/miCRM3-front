...
export class AppComponent implements OnInit {
	...
	// lpaa --> no permite cerrar el navegador sin consultar
	ngOnDestroy(): void {
			window.removeEventListener('beforeunload', this.handleBeforeUnload);
	}
	@HostListener('window:beforeunload', ['$event'])
	handleBeforeUnload(event: BeforeUnloadEvent): void {
		// Aquí puedes realizar acciones antes de que el navegador se cierre
		//console.log('cerrannnnnndo')
		event.preventDefault();
		(event as any).returnValue = '';
	}


	// lpaa --> para que antes de cargar el login no se muestre nada de la app
	async checkAuth() {
	...
	// Aumentar el tiempo para que se vea bien el splash screen
	setTimeout(() => {
		this.isLoading = false;
	}, 1500); // Cambiado de 100ms a 1500ms (1.5 segundos)
	}
