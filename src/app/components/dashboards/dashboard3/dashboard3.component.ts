...
/**
 * Copia standalone de modules/demo/pages/dashboard3.
 *
 * Al ser standalone declara aquí lo que usa su plantilla, y con eso basta
 * para que la pantalla de inicio lo cargue con @defer sin módulo de por
 * medio: el trozo se descarga sólo cuando se pulsa su pestaña.
 */
@Component({
  ...
  standalone: true,
  imports: [
    FormsModule,           // [(ngModel)] del selector de fechas
    NgApexchartsModule,    // <apx-chart>
    NgxDaterangepickerMd   // directiva ngxDaterangepickerMd
  ],
  providers: [
    // Mismo apaño que en auditoria-modal: un standalone no admite
    // NgxDaterangepickerMd.forRoot() en sus imports, y LocaleService no es
    // providedIn:'root', así que hay que declarar aquí lo que forRoot() provee.
    // Sin esto la directiva falla con NG0201.
...
ranges: any = {
  ...
  ngOnInit() {
  this.toastr.success("Exito",'Se cargo correctamente.');
...
ngModelChange2(e) {
  ...
  // Usar los valores directamente del objeto moment, sin toDate()
  const startDate = moment({
    year: e.startDate.year(),
    month: e.startDate.month(),
    day: e.startDate.date()
  }).locale('es');
  
  const endDate = moment({
    year: e.endDate.year(),
    month: e.endDate.month(),
    day: e.endDate.date()
  }).locale('es');
  ...
  // Calcular fechas previas
  const prevStart = moment(startDate).subtract('days', gap + 1);
  const prevEnd = moment(startDate).subtract('days', 1);
  
  // Formatear prevDate
  this.prevDate = prevStart.format('D MMMM') + ' - ' + prevEnd.format('D MMMM YYYY');
  
  // Formatear fechas actuales
  const start = startDate.format('D MMMM YYYY');
  const end = endDate.format('D MMMM YYYY');
  
  console.log('Rango seleccionado:', start, '-', end);
  console.log('Rango previo:', this.prevDate);
  
  // Actualizar input
  setTimeout(() => {
    const inputElement = document.querySelector('input[ngxdaterangepickermd]') as any;
    if (inputElement) {
...
ngModelChange(e) {
  ...
  // Extraer las fechas incluyendo horas
  const startDate = moment({
    year: e.startDate.year(),
    month: e.startDate.month(),
    day: e.startDate.date(),
    hour: e.startDate.hour(),
    minute: e.startDate.minute()
  }).locale('es');
  ...
  const endDate = moment({
    ...
    day: e.endDate.date(),
    hour: e.endDate.hour(),
    minute: e.endDate.minute()
  }).locale('es');
  
  // Calcular gap
  var gap = Math.round(endDate.diff(startDate, 'days', true));
  console.log('Gap:', gap);
  
  // Calcular fechas previas (manteniendo horas)
  const prevStart = moment(startDate).subtract('days', gap + 1);
  const prevEnd = moment(startDate).subtract('days', 1);
  
  // Formatear prevDate (solo fechas, sin horas)
  this.prevDate = prevStart.format('D MMMM') + ' - ' + prevEnd.format('D MMMM YYYY');
  
  // Formatear fechas actuales con horas
  const start = startDate.format('D MMMM YYYY HH:mm');
  const end = endDate.format('D MMMM YYYY HH:mm');
