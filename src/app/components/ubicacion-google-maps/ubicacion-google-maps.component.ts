...
import {
  ...
  Input, NgZone, OnDestroy, Output, ViewChild,
} from '@angular/core';
 ...
 */
/**
 * Mapa de Google: buscar un sitio, marcarlo en el mapa, ver la calle y
 * quedarse con sus datos.
 *
 * Copiado de la pantalla de demostración y convertido en componente suelto
 * para poder usarlo en cualquier parte del sistema —la ficha de un cliente,
 * la dirección de una sucursal, el punto de una entrega—. Se incrusta así:
 *
 *   <app-ubicacion-google-maps
 *       [lugarInicial]="{ lat: -2.17, lng: -79.92 }"
 *       (lugarElegido)="guardarDireccion($event)">
 *   </app-ubicacion-google-maps>
 *
 * La pantalla original de demo/ubicaciongooglemaps se queda como estaba.
 */
@Component({
...
export class UbicacionGoogleMapsComponent implements AfterViewInit, OnDestroy {
  ...
  /** Dónde abrirlo. Si no se dice nada, arranca donde lo hacía la demo. */
  @Input() lugarInicial: { lat: number; lng: number; nombre?: string } | null = null;

  /** Oculta el bloque de capturas cuando sólo se quiere elegir un punto. */
  @Input() mostrarCapturas = true;

  /** Alto del mapa; admite cualquier medida de CSS. */
  @Input() altoMapa = '';

  /**
   * Guarda la posición en la barra de direcciones, como hace la pantalla de
   * demostración. Apagado a propósito: incrustado en un formulario, escribir
   * en la URL le cambiaría la ruta a la pantalla que lo contiene.
   */
  @Input() sincronizarConLaUrl = false;

  /** El sitio que el usuario acaba de elegir o pulsar. */
  @Output() lugarElegido = new EventEmitter<LugarGm>();
  ...
  private inicializarMapa(): void {
    // Con un lugar de partida se abre encima de él; si no, sobre el país
    const centro = this.lugarInicial
      ? { lat: this.lugarInicial.lat, lng: this.lugarInicial.lng }
      : { lat: -1.8312, lng: -78.1834 };   // Ecuador

    this.mapa = new this.gm.Map(this.mapContainer.nativeElement, {
      center: centro,
      zoom: this.lugarInicial ? 16 : 7,
  ...
  private mostrarLugar(l: LugarGm): void {
    this.lugar = l;
    // Aquí pasa todo lo que elige el usuario: buscar, pulsar el mapa, «mi
    // ubicación»… así que es el punto donde se avisa a quien nos incrustó
    this.lugarElegido.emit(l);
    if (!this.mapa) { return; }
  ...
  private sincronizarUrl(): void {
    // Incrustado en otra pantalla no se toca la barra de direcciones: sería
    // pisarle la ruta a quien nos contiene
    if (!this.sincronizarConLaUrl) { return; }
  ...
  private aplicarParametrosUrl(): void {
    // Sin sincronización manda lo que diga `lugarInicial`, si lo hay
    if (!this.sincronizarConLaUrl) {
      if (this.lugarInicial) {
        this.resolverPunto(this.lugarInicial.lat, this.lugarInicial.lng);
      }
      return;
    }
  ...
  /**
   * Las dos fotos del lugar, pase lo que pase.
   *
   * `capturarCalle` sólo sirve con Street View abierto, y la del mapa
   * depende de que el usuario haya movido algo. Quien incruste el componente
   * para quedarse con la dirección necesita las dos sin pedirle nada más al
   * usuario, así que aquí se completa lo que falte.
   */
  async asegurarCapturas(): Promise<void> {
    if (!this.lugar) { return; }

    if (!this.capturas.some(c => c.tipo === 'mapa' && c.blob)) {
      await this.capturarMapa({ lat: this.lugar.lat, lng: this.lugar.lng });
    }

    if (!this.capturas.some(c => c.tipo === 'calle' && c.blob)) {
      await this.capturarCalleDelLugar();
    }
  }

  /**
   * La foto de la fachada sin abrir Street View: se le pide a la API estática
   * la vista desde la calle más cercana al punto.
   */
  private async capturarCalleDelLugar(): Promise<void> {
    if (!this.lugar) { return; }

    const { ancho, alto } = this.tamanoCaptura();
    const p = new URLSearchParams();
    p.set('location', `${this.lugar.lat.toFixed(6)},${this.lugar.lng.toFixed(6)}`);
    p.set('size', `${ancho}x${alto}`);
    p.set('scale', '2');
    // Sin panorama abierto no hay ángulo que copiar: Google apunta al sitio
    p.set('fov', '80');
    ...
    const captura: CapturaMapa = {
      ...
      origen: 'estatica',
      tipo: 'calle',
      url: `https://maps.googleapis.com/maps/api/streetview?${p.toString()}`,
      blob: null,
      ancho,
      alto,
      fallo: false,
    ...
    try {
      const r = await fetch(captura.url);
      if (!r.ok) { throw new Error('HTTP ' + r.status); }
      const blob = await r.blob();
      if (!blob.type.startsWith('image/')) { throw new Error('respuesta sin imagen'); }
      captura.blob = blob;
      ...
      // Puede no haber cobertura de Street View en ese punto: no es un fallo
      console.warn('No se pudo traer la vista de calle del lugar:', e);
      captura.fallo = true;
    }
